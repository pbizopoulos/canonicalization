...
import System.Directory (canonicalizePath, createDirectoryIfMissing, doesDirectoryExist, doesFileExist, findExecutable, getHomeDirectory, listDirectory, removeFile, setCurrentDirectory)
...
applyAutomaticFixes :: IO ()
applyAutomaticFixes = do
  packageNames <- listPackageNames
  changedPathGroups <- forM packageNames $ \packageName -> do
    kind <- detectProjectKindForPackage packageName
    createdCoverage <- ensureCoverageCheckForPackage packageName kind
    fixedPython <- fixPythonInstallCheckIfNeeded packageName kind
    let coveragePathToStage =
          case coverageCheckPathForKind packageName kind of
            Just path -> [path]
            Nothing -> []
...
coverageCheckPathForKind packageName kind =
  case kind of
    HaskellKind -> Just checkPath
    RustKind -> Just checkPath
    PythonKind -> Just checkPath
...
stageChangedPaths paths =
  unless (null paths) $ do
    (exitCode, _stdoutText, stderrText) <- readProcessWithExitCode "git" ("add" : "--" : paths) ""
    case exitCode of
      ExitSuccess -> pure ()
      ExitFailure _ -> do
        putStrLn ("failed to stage autogenerated fixes with git add: " ++ oneLine (T.pack stderrText))
        exitFailure
...
ensureCoverageCheckForPackage packageName kind = do
  ...
  if exists
    then pure []
      ...
      let maybeContents =
            case kind of
              HaskellKind -> Just (haskellCoverageCheck packageName)
              RustKind -> Just (rustCoverageCheck packageName)
              PythonKind -> Just (pythonCoverageCheck packageName)
              PythonLatexKind -> Just (pythonLatexCoverageCheck packageName)
              _ -> Nothing
      case maybeContents of
        Nothing -> pure []
        Just contents -> do
          createDirectoryIfMissing True checkDir
          TIO.writeFile checkFile contents
          pure [checkFile]
haskellCoverageCheck :: FilePath -> T.Text
haskellCoverageCheck packageName =
  T.unlines
    [ "{",
      "  pkgs,",
      "  ...",
      ...
      "  packageName = \"" <> T.pack packageName <> "\";",
...
rustCoverageCheck :: FilePath -> T.Text
rustCoverageCheck packageName =
  T.unlines
    [ "{",
      "  inputs,",
      "  pkgs,",
...
pythonCoverageCheck :: FilePath -> T.Text
pythonCoverageCheck packageName =
  T.unlines
    [ "{",
      "  pkgs,",
      "  ...",
...
pythonLatexCoverageCheck :: FilePath -> T.Text
pythonLatexCoverageCheck packageName =
  T.unlines
    [ "{",
      "  pkgs,",
      "  ...",
...
fixPythonInstallCheckIfNeeded :: FilePath -> ProjectKind -> IO [FilePath]
fixPythonInstallCheckIfNeeded packageName kind =
  if kind `elem` [PythonKind, PythonLatexKind]
    then do
      let path = "packages" </> packageName </> "default.nix"
      exists <- doesFileExist path
      if not exists
        then pure []
        else do
          contents <- TIO.readFile path
          let fixed = normalizePythonInstallCheckText contents
          if fixed /= contents
            then TIO.writeFile path fixed >> pure [path]
            else pure []
    else pure []
fixPythonTemplateFilesIfPresent :: IO [FilePath]
fixPythonTemplateFilesIfPresent = do
  let paths =
        [ "packages" </> "python_template" </> "default.nix",
          "packages" </> "python_latex_template" </> "default.nix"
        ]
  catMaybes <$> mapM fixFile paths
  where
    fixFile path = do
      exists <- doesFileExist path
      if not exists
        then pure Nothing
          ...
          let fixed = normalizePythonInstallCheckText contents
          if fixed /= contents
            then TIO.writeFile path fixed >> pure (Just path)
            else pure Nothing
normalizePythonInstallCheckText :: T.Text -> T.Text
normalizePythonInstallCheckText contents =
  ...
  where
    removeAttrBlock key = go False
      where
        ...
        endsBlock line = "];" `T.isSuffixOf` T.strip line
        go _ [] = []
        go False (line : rest)
...
removeTemporaryDefaultNixFiles :: IO ()
removeTemporaryDefaultNixFiles = do
  allPaths <- collectRepoPaths "."
  let tmpPaths =
        [ path
        | path <- allPaths,
          "packages/" `isPrefixOf` path,
          "default.nix.tmp" `isSuffixOf` path
        ]
  forM_ tmpPaths removeFileIfExists
...
runTuiMode = do
  ...
  case repos of
    [] -> do
      putStrLn "no valid git repositories found from $HOME/.gitmodules path entries"
      exitFailure
    _ -> pure ()
  (finalResultsByRepo, _finalState) <- runTui repos
  failOnIssues False (mergeCheckResults (Map.elems finalResultsByRepo))
...
buildCheckTree rootNodeId coverageByPackage availableCoveragePackages results =
  let packageLayout = buildPackageLabelLayout coverageByPackage availableCoveragePackages (packageResults results)
      packageNodes = map (packageNode rootNodeId coverageByPackage availableCoveragePackages packageLayout) (packageResults results)
      hostNodes = map (hostNode rootNodeId) (hostResults results)
...
packageNode :: String -> Map.Map FilePath Double -> Set.Set FilePath -> PackageLabelLayout -> PackageCheck -> TreeNode
packageNode repoNodeId coverageByPackage availableCoveragePackages layout pkg =
  let packageId = repoNodeId ++ "/packages/" ++ packageNodeName pkg
      testNodes = map (buildTestTreeNode coverageByPackage packageId (packageNodeName pkg)) (packageTests pkg)
      fileIssueNodes = buildPackageFileIssueNodes packageId pkg (Set.fromList (map testName (packageTests pkg)))
...
hostNode :: String -> HostCheck -> TreeNode
hostNode repoNodeId host =
  TreeNode
    { nodeId = repoNodeId ++ "/hosts/" ++ hostName host,
      nodeLabel = hostName host,
      nodeStatus = hostNodeStatus host,
      nodeChildren = map (buildTestTreeNode Map.empty (repoNodeId ++ "/hosts/" ++ hostName host) (hostName host)) (hostTests host)
    }
...
clearPendingG state = state {uiPendingG = False, uiPendingFixConfirm = Nothing, uiShowHelp = False, uiNotice = Nothing}
clearPendingZ :: UiState -> UiState
clearPendingZ state = state {uiPendingZ = False, uiPendingFixConfirm = Nothing, uiShowHelp = False, uiNotice = Nothing}
clearPendingFixConfirm :: UiState -> UiState
clearPendingFixConfirm state = state {uiPendingFixConfirm = Nothing, uiShowHelp = False, uiNotice = Nothing}
...
resetUiForRepoResults repo refreshedResults state =
  let repoPath = gitmoduleRepoPath repo
      nextResultsByRepo = Map.insert repoPath refreshedResults (uiResultsByRepo state)
      currentCoverageByRepo = uiCoverageByRepoPackage state
      nextCoverageByRepo =
        if Map.member repoPath currentCoverageByRepo
          then Map.insert repoPath Map.empty currentCoverageByRepo
          else currentCoverageByRepo
   in rebuildTree
        state
          { uiResultsByRepo = nextResultsByRepo,
            uiCoverageByRepoPackage = nextCoverageByRepo,
            uiViewportTop = 0,
            uiSelected = 0,
            uiPendingG = False,
            uiPendingZ = False,
...
mergeCheckResults repoResults =
  CheckResults
    { structureIssues = concatMap structureIssues repoResults,
      packageResults = concatMap packageResults repoResults,
      hostResults = concatMap hostResults repoResults
    }
...
projectKindLabel kind =
  case kind of
    ...
    UnknownKind -> "unknown"
buildPackageFileIssueNodes :: String -> PackageCheck -> Set.Set String -> [TreeNode]
buildPackageFileIssueNodes packageId pkg representedFiles =
  let grouped = foldl collect Map.empty (filter (not . isDirectoryStructureIssue) (packageErrors pkg))
   ...
   in [ TreeNode
          { nodeId = packageId ++ "/file/" ++ fileLabel,
            nodeLabel = fileLabel,
            nodeStatus = Failed,
            nodeChildren =
              [ TreeNode
                  { nodeId = packageId ++ "/file/" ++ fileLabel ++ "/issue/" ++ show i,
                    nodeLabel = msg,
                    nodeStatus = Failed,
                    nodeChildren = []
                  }
